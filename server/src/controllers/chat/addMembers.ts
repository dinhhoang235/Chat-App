  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    ...
    try {
      ...
      if (!participant || !participant.conversation.isGroup) {
        return res
          .status(403)
          .json({ message: "Only group members can add new members" });
      }
      ...
      const existingUserIds = existingParticipants.map(
      ...
      if (updatedConversation) {
        ...
        updatedConversation.participants.forEach(
      ...
      return res
        .status(200)
        .json({ success: true, addedCount: newUserIds.length });
    } catch (err) {
