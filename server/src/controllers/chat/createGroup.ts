  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    ...
    try {
      ...
      const allParticipantIds = Array.from(
        new Set([...pIds.map((id) => parseInt(id)), userId]),
      );
      ...
      // Fire-and-forget: generate composite avatar in background
      generateComposite(conversation.id)
        .then((res) => {
          if (res) {
            // Emit update to participants to pick up new composite URL
            allParticipantIds.forEach((id) => {
              io.to(`user:${id}`).emit("conversation_updated", {
                conversationId: conversation.id,
                action: "composite_updated",
                compositeAvatarUrl: res.url,
                compositeAvatarVersion: res.version,
              });
            });
          }
        })
        .catch((err) =>
          console.error("Background composite generation failed", err),
        );
