  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    ...
    try {
      ...
      if (!participant.conversation.isGroup) {
        return res
          .status(400)
          .json({ message: "Only groups can be disbanded" });
      }

      if (participant.role !== "owner" && participant.role !== "admin") {
        return res
          .status(403)
          .json({ message: "Only owner or admin can disband the group" });
      }
      ...
      allParticipants.forEach((p: { userId: number }) => {
        io.to(`user:${p.userId}`).emit("conversation_disbanded", {
          conversationId: convId,
        });
      });
