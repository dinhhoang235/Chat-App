  ...
  async (req: AuthRequest, res: Response): Promise<any> => {
    ...
    try {
      ...
      const targetUserId = parseInt(
        Array.isArray(targetUserIdStr) ? targetUserIdStr[0] : targetUserIdStr,
      );

      if (requesterId === targetUserId) {
        return res
          .status(400)
          .json({ message: "Cannot remove yourself using this endpoint" });
      }
      ...
      if (requester.role === "admin") {
        if (target.role === "owner") {
          return res
            .status(403)
            .json({ message: "Admins cannot remove the owner" });
        }
