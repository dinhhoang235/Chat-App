...
if (
  ...
  console.warn(
    "⚠️ CẢNH BÁO: Thiếu các biến môi trường cho MinIO. Kiểm tra lại file .env",
  );
}
...
export const bucketName = config.bucketName || "chatapp";
...
export const getPresignedUrl = async (
  ...
  return rewrittenUrl
    .replace(requestHostResult, `${requestHostResult}/storage`)
    .replace(`:${config.port}`, "");
};
...
export const initMultipartUpload = async (
  ...
  try {
    ...
    const method =
      ...
      minioClient.initiateMultipartUpload;
    ...
    // @ts-ignore
    const uploadId = await method.call(
      minioClient,
      bucketName,
      objectName,
      metadata || {},
...
export const getPresignedUrlForPart = async (
  objectName: string,
  uploadId: string,
  partNumber: number,
  requestHost?: string,
...
export const abortMultipartUpload = async (
  ...
  throw new Error(
    "Abort multipart upload is not supported by current MinIO SDK",
  );
};
